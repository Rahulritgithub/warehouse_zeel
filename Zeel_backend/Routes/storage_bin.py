...
def update_storage_bin(rfid: str, storage_bin_update: StorageBinUpdate, db: Session = Depends(get_db)):
    ...
    if storage_bin_update.rfid is not None:
        # Check if new RFID already exists (if changing RFID)
        if storage_bin_update.rfid != storage_bin.rfid:
            existing_bin = db.query(StorageBin).filter(
                StorageBin.rfid == storage_bin_update.rfid
            ).first()
            if existing_bin:
                raise HTTPException(status_code=400, detail="RFID already exists")
        storage_bin.rfid = storage_bin_update.rfid
    
    if storage_bin_update.rack_id is not None:
        storage_bin.rack_id = storage_bin_update.rack_id
    
    if storage_bin_update.capacity is not None:
        if storage_bin_update.capacity <= 0:
            raise HTTPException(status_code=400, detail="Capacity must be greater than 0")
        storage_bin.capacity = storage_bin_update.capacity
